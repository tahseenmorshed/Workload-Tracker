...
const UserRow = ({ data, onSelect, selectedIds, onEdit, suspendedIds, }) => 
{
  ...
  const handleEditClick = (item) => {
    onEdit(item);
    navigate(`/uniteditpage/${item.code}`, { state: { unitData: item } });
  };
      ...
      <Table.Row>
        ...
        (  // Maps over data array to render a row for each staff member 
           <Table.Row // Creates table row with unique key 
            key={item.code}
            className={suspendedIds.has(item.code) ? 'suspended' : ''}>
              <Table.Cell> {/* Table cell containing a checkbox to select the staff member*/}
              <Form.Check
                type="checkbox"
                checked={selectedIds.has(`${item.code}-${item.studyPeriod}`)}
                onChange={() => onSelect(item.code, item.studyPeriod)}
              ...
              />
                ...
                <Table.Cell>
                <Link to={`/uniteditpage/${item.code}`} onClick={() => handleEditClick(item)}>
                  <FaPencilAlt
                    variant="info"
                    color="black"
